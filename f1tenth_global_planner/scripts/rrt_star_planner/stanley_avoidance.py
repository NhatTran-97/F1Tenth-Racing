...
class StanleyAvoidance(Node):
    def __init__(self):
        ...
        self.declare_parameter("waypoints_path", "/home/fablab_01/f1_ws/src/f1tenth_projects/f1tenth_waypoint/racelines/f1tenth_waypoint_path.csv")
        self.declare_parameter("waypoints_path_2nd", "/home/fablab_01/f1_ws/src/f1tenth_projects/f1tenth_waypoint/racelines/f1tenth_waypoint_path.csv")
...
class WaypointUtils:
    def __init__(
        ...
        filepath="/home/fablab_01/f1_ws/src/f1tenth_projects/f1tenth_navigation/src/controller/racelines/e7_floor5.csv",
        min_lookahead=0.5,
        max_lookahead=3.0,
        min_lookahead_speed=3.0,
        max_lookahead_speed=6.0,
        filepath_2nd="/home/fablab_01/f1_ws/src/f1tenth_projects/f1tenth_navigation/src/controller/racelines/e7_floor5.csv",
        lane_number=0,
    ):
    ...
    def load_and_interpolate_waypoints(self, file_path, interpolation_distance=0.05):
        try:
            ...
            if np.isnan(points).any() or np.isinf(points).any():
                raise ValueError("Found NaN or Inf in waypoint coordinates.")

            if np.isnan(velocities).any() or np.isinf(velocities).any():
                raise ValueError("Found NaN or Inf in waypoint velocities.")
            ...
            if np.isnan(total_distance) or interpolation_distance == 0:
                raise ValueError("Invalid total_distance or interpolation_distance detected.")
            ...
            self.node.get_logger().error(f"Error in loading waypoints: {e}")
            raise
