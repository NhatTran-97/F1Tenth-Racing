...
def plot_box(image, top_left_point, bottom_right_point, width, height, label, color=(210,240,0), padding=6, font_scale=0.35):
    ...
    # p3 = bottom_right_point['x'], top_left_point['y'] - font_height - padding*2
    ...
    # cv2.rectangle(image, (48, 100), (200, 300), (255, 255, 255), -1, lineType=cv2.LINE_AA)
    x = top_left_point['x'] + padding
    y = top_left_point['y'] - padding
    # cv2.putText(image, label, (x, y), cv2.FONT_HERSHEY_PLAIN, 0.95, [0, 0, 0], thickness=2, lineType=cv2.LINE_AA)
    cv2.putText(image, label, (x, y), font_face, 1.0, [0, 0, 255], thickness=1, lineType=cv2.LINE_AA)
